...
class TestMessagingServiceIntegration:
    ...
    def test_submit_message(self, sample_message1):
    ...
    def test_submit_message_validation_error_empty_recipient(self):
    ...
    def test_fetch_new_messages_includes_new_message(self):
        """Fetch new messages should include messages sent after the request"""

        unique_recipient = f"test_{int(time.time())}@example.com"
        message1 = {"content": "message 1", "recipient_id": unique_recipient}
        message2 = {"content": "message 2", "recipient_id": unique_recipient}
    ...
    def test_delete_message(self, sample_message1):
        ...
        # Send a message first (using sync endpoint because it is easier to test)
        ...
        fetch_response = requests.get(f"{BASE_URL}/messages/{unique_recipient}?start={message_id}&stop={message_id}")
    ...
    def test_fetch_multiple_messages_with_pagination(self):
        ...
        fetch_response = requests.get(f"{BASE_URL}/messages/{unique_recipient}?start=1&stop=9999")
        assert fetch_response.status_code == 200
    ...
    def test_fetch_multiple_messages_pagination_range(self):
        ...
        for i in range(5):
            ...
            response = requests.post(f"{BASE_URL}/messages/sync", json=message)
            if i==0:
                start = response.json()["id"]
            if i==2:
                stop = response.json()["id"]

        # Fetch messages from first to third
        fetch_response = requests.get(f"{BASE_URL}/messages/{unique_recipient}?start={start}&stop={stop}")
        assert fetch_response.status_code == 200
